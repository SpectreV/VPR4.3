 ...
 * oversubscribed rr_nodes are penalized.                                    */
 ...
 while (1) {
    inode = tptr->index;
    // if (add_or_sub == 1)
    //  printf("index %d cost_index %d\n", inode, rr_node[inode].cost_index);
 ...
 for (i=0;i<num_nets;i++)
   {
     if (is_folding){
       if(current_stage>0)
	 {if ((i>=num_net_per_stage[current_stage-1].begin)&&(i<num_net_per_stage[current_stage-1].begin+num_net_per_stage[current_stage-1].num))
	   free_traceback (i);
	 }
       else
	 free_traceback (i);
 ...
 * timing-driven router -- the breadth-first router ignores them.           */
 ...
 if (cost >= rr_node_route_inf[inode].path_cost||cost>0.99*HUGE_FLOAT)
     return;
 ...
 for (iblk=0;iblk<num_blocks;iblk++) {
    if (block[iblk].type != CLB) {
     ...
     /* Subblock output used only locally, but must connect to a CLB OPIN?  */
       ...
       for (iff=0;iff<num_ff_per_block[iblk];iff++) {
          clb_pin = ffblock_inf[iblk][iff].output;
     /* Subblock output used only locally, but must connect to a CLB OPIN?  */
          if (clb_pin != OPEN && block[iblk].nets[clb_pin] == OPEN) {
             iclass = clb_pin_class[clb_pin];
             clb_opins_used_locally[iblk][iclass].nelem++;
...
/* Prints out the routing to file route_file.  */
 ...
 t_rr_type rr_type;
 struct s_trace *tptr;
 char *name_type[] = {"SOURCE", "SINK", "IPIN", "OPIN", "CHANX","CHANY", "DIREX", "DIREY"};
 FILE *fp;
 int istage; /*Added by Wei */
 ...
 for (inet=0;inet<num_nets;inet++) {
   if (is_folding){
     if(istage!=net[inet].stage)
       ...
       fprintf(fp, "The folding stage: %d", istage);
       }
   }
    ...
    if (is_global[inet] == FALSE) {
       ...
       while (tptr != NULL) {
          ...
          case IPIN: case OPIN:
             if (clb[ilow][jlow].type == CLB || clb[ilow][jlow].type == MEM) {
                fprintf (fp, " Pin: ");
             }
             else {   /* IO Pad. */
                fprintf (fp, " Pad: ");
          ...
          case CHANX: case CHANY: case DIREX: case DIREY:
             fprintf (fp, " Track: "); 
             break;
 ...
 * reserved.                                                              */
 ...
 if (rip_up_local_opins) {
   for (iblk=0;iblk<num_blocks;iblk++) {
	 ...
	 if ((iblk<num_block_per_stage[current_stage-1].begin)||(iblk>=num_block_per_stage[current_stage].begin))
	   continue;
       } 
       else {
