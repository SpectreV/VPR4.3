 ...
 * (x_clb_left[nx+1]+clb_width, y_clb_bottom[ny+1]+clb_width) in t he      *
 * upper right corner.                                                    */
...
static void draw_rr_direx (int inode, int itrack);
static void draw_rr_direy (int inode, int itrack);
...
/* Highlights all the blocks and nets on the critical path.                 */
 ...
 critical_path_head = allocate_and_load_critical_path (current_stage, TRUE);
 ...
 if (clb[i][j].type == CLB || clb[i][j].type == MEM) {
   ...
   if (!is_folding){
    for (k=0;k<clb[i][j].occ;k++) 
       ...
       for (k=0;k<stage_clb[current_stage-1][i][j].occ;k++) 
       if (stage_clb[current_stage-1][i][j].u.io_blocks[k] == bnum) 
          break;
     }
...
/* Draws all the overused routing resources (i.e. congestion) in RED.   */
 ...
 for (inode=0;inode<num_rr_nodes;inode++) {
    if (rr_node[inode].occ > rr_node[inode].capacity) { 
       ...
       case DIREX:
	 itrack = rr_node[inode].ptc_num;
	 draw_rr_direx (inode, itrack);
	 break;
       case DIREY:
	 itrack = rr_node[inode].ptc_num;
	 draw_rr_direy (inode, itrack);
	 break;
       case IPIN: case OPIN:
...
static void draw_rr_direx (int inode, int itrack) {
  ...
  x1 = x_clb_left[rr_node[inode].xlow]+clb_width;
  x2 = x_clb_left[rr_node[inode].xlow+1];
  y = y_clb_bottom[rr_node[inode].ylow] + 1.0 + itrack;
  drawline (x1, y, x2, y);
}
...
static void draw_rr_direy (int inode, int itrack) {
  ...
  y1 = y_clb_bottom[rr_node[inode].ylow]+clb_width;
  y2 = y_clb_bottom[rr_node[inode].ylow+1];
  x = x_clb_left[rr_node[inode].xlow] + 1.0 + itrack;
  drawline (x, y1, x, y2);
}
 ...
 * the rr graph).                                                            */
 ...
 case DIREX:
   ...
   chan_ylow = rr_node[chan_node].ylow;
   mid_chan_x = (x_clb_left[rr_node[chan_node].xlow+1] - x_clb_left[rr_node[chan_node].xlow]-clb_width)/2.0;
   if (pinloc[TOP][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, TOP, &x1, &y1);
   else if (pinloc[BOTTOM][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, BOTTOM, &x1, &y1);
   else if (pinloc[RIGHT][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, RIGHT, &x1, &y1);
   else if (pinloc[LEFT][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, LEFT, &x1, &y1);
   else
     {printf("error: pin not on either side\n");
     exit(1);}
   y2 = y_clb_bottom[chan_ylow] + 1 + itrack;
   x2 = x_clb_left[rr_node[chan_node].xlow]+clb_width+mid_chan_x;
   break;
 
  case DIREY:

   chan_xlow = rr_node[chan_node].xlow;
   mid_chan_y = (y_clb_bottom[rr_node[chan_node].ylow+1] - y_clb_bottom[rr_node[chan_node].ylow]-clb_width)/2.0;
   if (pinloc[TOP][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, TOP, &x1, &y1);
   else if (pinloc[BOTTOM][pin_num]==1)
     get_rr_pin_draw_coords (pin_node, BOTTOM, &x1, &y1);
     ...
     {printf("error: pin not on either side\n");
     exit(1);}
   x2 = x_clb_left[chan_xlow] + 1 + itrack;
   y2 = y_clb_bottom[rr_node[chan_node].ylow]+clb_width+mid_chan_y;
   break; 
 ...
 * current color to BLACK.                                                 */
 ...
 if (clb[i][j].type == CLB || clb[i][j].type == MEM) {
   ...
   if (!is_folding)
    ...
    iblk = stage_clb[current_stage-1][i][j].u.io_blocks[ipin];
 ...
 * that are not coloured black (useful for drawing over the rr_graph).       */
 ...
 static int **direx_track = NULL;           /* [1..nx-1][1..ny] */
 static int **direy_track = NULL;           /* [1..nx][1..ny-1] */
 ...
 if (draw_route_type == GLOBAL) {
   /* Allocate some temporary storage if it's not already available. */
    ...
    if (direx_track == NULL) 
       direx_track = (int **) alloc_matrix (1, nx-1, 1, ny, sizeof(int));

    if (direy_track == NULL) 
       direy_track = (int **) alloc_matrix (1, nx, 1, ny-1, sizeof(int));
...
/* Now draw each net, one by one.      */
 ...
 for (inet=0;inet<num_nets;inet++) {
    ...
    while (1) {
       ...
       case DIREX:
	  if (draw_route_type == GLOBAL)
             direx_track[rr_node[inode].xlow][rr_node[inode].ylow]++;

          itrack = get_track_num (inode, direx_track, direy_track);
          draw_rr_direx (inode, itrack);
	 ...
	 if (draw_route_type == GLOBAL)
             direy_track[rr_node[inode].xlow][rr_node[inode].ylow]++;
...
/* GLOBAL route stuff below. */
 ...
 switch (rr_type) {
 case CHANX: case DIREX:
    return (chanx_track[i][j]);

 case CHANY: case DIREY:
...
/* The user selected the clb at location (i,j). */
 if (clb[i][j].type == CLB || clb[i][j].type == MEM) {
   if(!is_folding){
     ...
     if (stage_clb[current_stage-1][i][j].occ == 0) {
       update_message (default_message); 
       drawscreen ();
       return;
     }
    bnum = stage_clb[current_stage-1][i][j].u.block;
   }
 }
 else {   /* IO block clb */
    ...
    if(is_folding){
      if (k >= stage_clb[current_stage-1][i][j].occ) {   /* Empty spot */
	update_message (default_message); 
	drawscreen();
	return;
      }
      bnum = stage_clb[current_stage-1][i][j].u.io_blocks[k];
      if (bnum==EMPTY){
     	update_message (default_message); 
	drawscreen();
	return;
 ...
 * grey, while empty ones are left white and have a dashed border.      */
 ...
 for (i=1;i<=nx;i++) { 
    for (j=0;j<=ny+1;j+=ny+1) {  /* top and bottom */
	 ...
	 for (k=stage_clb[current_stage-1][i][j].occ;k<io_rat;k++) {
	   x1 = x_clb_left[i] + k * io_step;
	   x2 = x1 + io_step;
	   drawrect (x1,y1,x2,y2);
	 }
    ...
    for (i=0;i<=nx+1;i+=nx+1) {  /* IOs on left and right */
       ...
       for (k=0;k<clb[i][j].occ;k++) {
        ...
        for (k=0;k<stage_clb[current_stage-1][i][j].occ;k++) {
          ...
          drawrect (x1,y1,x2,y2);
          drawtext ((x1 + x2)/2., (y1 + y2)/2.,
          block[stage_clb[current_stage-1][i][j].u.io_blocks[k]].name, clb_width);
         }
         else{
          y1 = y_clb_bottom[j] + k * io_step;
          y2 = y1 + io_step;
 ...
 for (i=1;i<=nx;i++) {
    ...
    for (j=1;j<=ny;j++) {
       ...
       if (stage_clb[current_stage-1][i][j].occ != 0) {
          ...
          setcolor (BLACK);
          drawrect (x1,y1,x2,y2);
          drawtext ((x1 + x2)/2., (y1 + y2)/2., block[stage_clb[current_stage-1][i][j].u.block].name, 
             clb_width);
       }
       else {
          setlinestyle (DASHED);
