...
static int load_direx_rr_indices (int nodes_per_chan, int start_index, int i, int j); 

static int load_direy_rr_indices (int nodes_per_chan, int start_index, int i, int j); 
...
t_seg_details *alloc_and_load_direct_details (int nodes_direct, t_segment_inf 
              *segment_inf, int num_seg_types) {
  int itrack, i, type_index;
  float rmetal, cmetal;
  t_seg_details *seg_details;
  seg_details = (t_seg_details *) my_malloc (nodes_direct * sizeof (t_seg_details));
  //printf("num_seg %d\n", num_seg_types);
  for (i=0; i<num_seg_types; i++)
...
int get_clb_opin_connections (int **clb_ipin_to_direct, int ***clb_opin_to_tracks, int nodes_direct, int ipin, int i, int 
        j, int delayless_switch, int Fc_output, t_seg_details *seg_details_x, t_seg_details 
    ...
    // printf("come to direct links \n");
    {
      for (itrack=0; itrack<4*nodes_direct; itrack++)
      {
        if (clb_ipin_to_direct[ipin][itrack]==1)
        {
          ...
          //nionio modified
          if (to_rr_type == DIREX && tr_i >= 0 && tr_i <=nx && tr_j >0 && tr_j <=ny)
		      {
					  to_node = get_rr_node_index (tr_i, tr_j, to_rr_type, pin_num, nodes_per_chan, nodes_direct, io_rat, 
                    rr_node_indices);
					  edge_list_head = insert_in_edge_list (edge_list_head, to_node,
          ...
          //nionio modified
          if (to_rr_type == DIREY && tr_i > 0  && tr_i <=nx && tr_j >=0 && tr_j <=ny)
		      {
					  to_node = get_rr_node_index (tr_i, tr_j, to_rr_type, pin_num, nodes_per_chan, nodes_direct, io_rat,  
                    rr_node_indices);
					  edge_list_head = insert_in_edge_list (edge_list_head, to_node,
...
//nionio modified
int get_pad_opin_connections (int **pads_to_tracks, int *pads_to_direct, int ipad, int i, int j, 
        int Fc_pad, int nodes_direct, int delayless_switch, 
 ...
 if (nodes_per_chan!=0)
 {
   for (iconn=0;iconn<Fc_pad;iconn++) {
    ...
    if (if_direct)
    {
	   ...
	   to_node = get_rr_node_index (chan_i, chan_j, chan_type, itrack, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);

	   //add delayless_switch to input
       edge_list_head = insert_in_edge_list (edge_list_head, to_node, delayless_switch, &free_edge_list_head);
       num_conn++;
    }	
...
int **alloc_and_load_rr_node_indices (int nodes_per_clb,
    int nodes_per_pad, int nodes_per_chan, int nodes_direct, int io_rat,
    t_seg_details *seg_details_x, t_seg_details *seg_details_y) {
 ...
 * track in each (i,j) channel segment.                                    */ 
 ...
 {
   direx_rr_indices = (int ***) alloc_matrix3 (0, nx, 1, ny, 0, 2*2, sizeof (int));
   direy_rr_indices = (int ***) alloc_matrix3 (1, nx, 0, ny, 0, 2*2, sizeof (int));
 } 
     ...
     if (clb[i][j].type == CLB||clb[i][j].type == MEM) {
       ...
       if (j == 0) {   /* Bottom row */
         ...
         //nionio added
           ...
           if (nodes_direct > 2)
	  		     index = load_direy_rr_indices (nodes_direct, index, i, j);
           else index = load_direy_rr_indices (2, index, i, j);
         }		
	     }
       if (i == 0) {   /* Leftmost column */
         ...
         //nionio added
           ...
           //nionio 7/11
			     if (nodes_direct > 2)
             index = load_direx_rr_indices (nodes_direct, index, i, j);
           else index = load_direx_rr_indices (2, index, i, j);
         }			  
	     }
		 }
...
static int load_direx_rr_indices (int nodes_direct, int start_index, int i, int j) {
...
static int load_direy_rr_indices (int nodes_direct, int start_index, int i, int j) {
 ...
 * question exists.                                                         */
 ...
 case CLB: case MEM:
    ...
    case DIREX:
      ...
      if ((i==nx)||(i==0)) assert (ioff < 2*stage_io_rat);
      else assert (ioff < 2*nodes_direct);
    ...
    case DIREY:
      ...
      if ((j==ny)||(j==0)) assert (ioff < 2*stage_io_rat);
      else assert (ioff < 2*nodes_direct);
...
int get_direct_to_clb_ipin_edges (t_linked_edge **edge_list_ptr, int i, int j, int itrack,
				  int nodes_per_chan, int nodes_direct, int io_rat, int **clb_ipin_to_direct, int **rr_node_indices, 
 ...
 if (connect_all)
 {
   for (ipin =0; ipin<pins_per_clb; ipin++)
   {
     ...
     if (class_inf[iclass].type == RECEIVER) {
       to_node = get_rr_node_index (i, j, IPIN, ipin, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
	     edge_list_head = insert_in_edge_list(edge_list_head, to_node, wire_to_ipin_switch, &free_edge_list_head);
	     num_conn ++;
     }
   }
 } 
...
//nionio added
int get_direct_to_pad_edges (t_linked_edge **edge_list_ptr, int i, int j, int itrack, int nodes_per_chan,
				                     ...
				                     int *pads_to_direct, int **rr_node_indices, int wire_to_ipin_switch)
{
	...
	for(ipad=0; ipad<io_rat; ipad++)
	{
		...
		{
		    to_node = get_rr_node_index (i, j, IPIN, ipad, nodes_per_chan, nodes_direct, io_rat, rr_node_indices);
	        edge_list_head = insert_in_edge_list(edge_list_head, to_node, wire_to_ipin_switch, &free_edge_list_head);
	        num_conn ++;
		}
	}
...
int get_xtrack_to_clb_ipin_edges (int tr_istart, int tr_iend, int tr_j,
       ...
       **tracks_to_clb_ipin, int nodes_per_chan, int nodes_direct, int io_rat, 
       int **rr_node_indices, t_seg_details *seg_details_x, int wire_to_ipin_switch) {
 ...
 for (i=tr_istart;i<=tr_iend;i++) {
    if (is_cbox (i, tr_j, itrack, seg_details_x)) {
       for (iconn=0;iconn<max_conn;iconn++) {
          ...
          to_node = get_rr_node_index (i, clb_j, IPIN, ipin, nodes_per_chan, nodes_direct, io_rat, 
                    rr_node_indices);
          edge_list_head = insert_in_edge_list (edge_list_head, to_node,
...
int get_ytrack_to_clb_ipin_edges (int tr_jstart, int tr_jend, int tr_i,
       int itrack, int iside, t_linked_edge **edge_list_ptr, struct s_ivec
       **tracks_to_clb_ipin, int nodes_per_chan, int nodes_direct, int io_rat, int **rr_node_indices,
...
/* Side is from the *track's* perspective */
 ...
 for (j=tr_jstart;j<=tr_jend;j++) {
    if (is_cbox (j, tr_i, itrack, seg_details_y)) {
       for (iconn=0;iconn<max_conn;iconn++) {
          ipin = tracks_to_clb_ipin[itrack][iside].list[iconn];
          to_node = get_rr_node_index (clb_i, j, IPIN, ipin, nodes_per_chan, nodes_direct, io_rat, 
                    rr_node_indices);
          edge_list_head = insert_in_edge_list (edge_list_head, to_node, 
                        wire_to_ipin_switch, &free_edge_list_head);
       }
...
int get_xtrack_to_pad_edges (int tr_istart, int tr_iend, int tr_j, int pad_j,
        ...
        *tracks_to_pads, int nodes_per_chan, int nodes_direct, int io_rat, int **rr_node_indices, 
 ...
 for (i=tr_istart;i<=tr_iend;i++) {
    if (is_cbox (i, tr_j, itrack, seg_details_x)) {
       for (iconn=0;iconn<max_conn;iconn++) {
          ...
          to_node = get_rr_node_index (i, pad_j, IPIN, ipad, nodes_per_chan, nodes_direct, io_rat,  
                        rr_node_indices);
          edge_list_head = insert_in_edge_list (edge_list_head, to_node,
                           wire_to_ipin_switch, &free_edge_list_head);
       }
 ...
 for (j=tr_jstart;j<=tr_jend;j++) {
    if (is_cbox (j, tr_i, itrack, seg_details_y)) {
       for (iconn=0;iconn<tracks_to_pads[itrack].nelem;iconn++) {
          ipad = tracks_to_pads[itrack].list[iconn];
          to_node = get_rr_node_index (pad_i, j, IPIN, ipad, nodes_per_chan, nodes_direct, io_rat,  
                        rr_node_indices);
          edge_list_head = insert_in_edge_list (edge_list_head, to_node,
                           wire_to_ipin_switch, &free_edge_list_head);
       } 
...
int get_xtrack_to_ytracks (int from_istart, int from_iend, int from_j, int 
       from_track, int to_j, t_linked_edge **edge_list_ptr, int nodes_per_chan, int nodes_direct, int io_rat,
...
int get_ytrack_to_xtracks (int from_jstart, int from_jend, int from_i, int 
       from_track, int to_i, t_linked_edge **edge_list_ptr, int nodes_per_chan, int nodes_direct, int io_rat,
       int **rr_node_indices, t_seg_details *seg_details_x, t_seg_details 
       *seg_details_y, enum e_switch_block_type switch_block_type) {
...
int get_xtrack_to_xtrack (int from_i, int j, int from_track, int to_i, 
       t_linked_edge **edge_list_ptr, int nodes_per_chan, int nodes_direct, int io_rat, int 
 ...
 * of edges and switch types.                                               */
 ...
 for (iconn=0;iconn<conn_tracks.nelem;iconn++) {
    ...
    if (switch_types[0] != OPEN) {
       to_node = get_rr_node_index (to_i, j, CHANX, to_track, nodes_per_chan, nodes_direct, io_rat, 
               rr_node_indices);
...
if (from_j < to_j) { 
 ...
 for (iconn=0;iconn<conn_tracks.nelem;iconn++) {
    ...
    if (switch_types[0] != OPEN) { 
       to_node = get_rr_node_index (i, to_j, CHANY, to_track, nodes_per_chan, nodes_direct, io_rat, 
                rr_node_indices); 
