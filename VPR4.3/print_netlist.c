...
void print_netlist(char *foutput, char *net_file, t_subblock_data subblock_data) {
	...
	fprintf(fp, "num_blocks: %d, num_nets: %d, num_globals: %d\n", num_blocks,
			num_nets, num_globals);
	fprintf(fp, "\nNet\tName\t\t#Pins\tDriver\t\tRecvs. (block, pin)\n");
	if (is_folding)
		fprintf(fp, "num_stages: %d\n", num_stage);

	istage = 0;
	for (i = 0; i < num_nets; i++) {
		if (is_folding) {
			if (istage != net[i].stage) {
				istage = net[i].stage;
				fprintf(fp, "\tIn stage: %d", istage);
			}
		}
		fprintf(fp, "\n%d\t%s\t", i, net[i].name);
	...
	for (i = 0; i < num_blocks; i++) {
		...
		if (block[i].type == INPAD || block[i].type == OUTPAD)
			max_pin = 1;
		else if (block[i].type == DSP) {
			max_pin = pins_per_dsp;
		} else
	...
	for (i = 0; i < num_blocks; i++) {
		...
		fprintf(fp, "\nBlock: %d (%s)\tNum_subblocks\tNum_ffblocks: %d\n", i,
				block[i].name, num_subblocks_per_block[i], num_ff_per_block[i]);
		...
		fprintf(fp, "Index\tInput\tOutput\tClock\n");

		for (j = 0; j < num_ff_per_block[i]; j++) {
			fprintf(fp, "%d\t%d", j, ffblock_inf[i][j].index);
			print_pinnum(fp, ffblock_inf[i][j].input);
			print_pinnum(fp, ffblock_inf[i][j].output);
			print_pinnum(fp, ffblock_inf[i][j].clock);
			fprintf(fp, "\n");
		}
	}
