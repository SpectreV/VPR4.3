...
static void build_rr_clb (int **rr_node_indices, int Fc_output, int *pin_to_opin, int ***
...
static void build_rr_xdir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
      int nodes_per_chan, int nodes_direct, t_seg_details *seg_direct_x, int wire_to_ipin_switch, int cost_index_offset);

static void build_rr_ydir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
      int nodes_per_chan, int nodes_direct, t_seg_details *seg_direct_y, int wire_to_ipin_switch, int cost_index_offset);
 ...
 * anything on this side, the [ipin][iside][0] element is OPEN.        */
 ...
 if(if_direct)
   {seg_direct_x = alloc_and_load_direct_details (nodes_direct, segment_inf,
						  det_routing_arch.num_segment);
   
   seg_direct_y = alloc_and_load_direct_details (nodes_direct, segment_inf,
						 det_routing_arch.num_segment);
   }
#ifdef DEBUG
 ...
 * just rr_node_indices are allocated by this routine.                     */
 ...
 rr_node_indices = alloc_and_load_rr_node_indices (nodes_per_clb, 
         nodes_per_pad, nodes_per_chan, nodes_direct, seg_details_x, seg_details_y);
 ...
 * track to connect to the same pin twice.                                   */
 ...
 //nionio added		  
 pads_to_direct = alloc_and_load_pads_to_direct (nodes_direct);

 /* noinio summarize
    till now: clb_ipin_to_tracks, clb_opin_to_tracks, tracks_to_clb_ipin,
              clb_ipin_to_direct,
 ...
 * allocated as I fill in the graph.                                    */
 ...
 for (i=0;i<=nx+1;i++) {
    for (j=0;j<=ny+1;j++) {
	   ...
	   printf("CLB(%d,%d) \n", i, j);
	 //printf("CLB %d %d\n", i, j);
         build_rr_clb (rr_node_indices, Fc_output, pin_to_opin, clb_opin_to_tracks, 
                 nodes_per_chan, nodes_direct, i, j, delayless_switch, seg_details_x, 
                 seg_details_y);
	 //printf ("come to x chan\n");
         ...
         build_rr_xdir (rr_node_indices, clb_ipin_to_direct, pads_to_direct, i, j, nodes_per_chan,
                        nodes_direct, seg_direct_x, wire_to_ipin_switch, CHANX_COST_INDEX_START);
	  // printf("come to y chan\n");
         ...
         build_rr_ydir (rr_node_indices, clb_ipin_to_direct, pads_to_direct, i, j, nodes_per_chan,
                        nodes_direct, seg_direct_y, wire_to_ipin_switch, CHANX_COST_INDEX_START + num_segment);
...
static void build_rr_clb (int **rr_node_indices, int Fc_output,
			  int *pin_to_opin, int ***clb_opin_to_tracks, 
...
/* Now do the pins.  */
 ...
 for (ipin=0;ipin<pins_per_clb;ipin++) {
    ...
    if (class_inf[iclass].type == DRIVER) {  /* OPIN */
       ...
       num_edges = get_clb_opin_connections (pin_to_opin, clb_opin_to_tracks, nodes_direct, ipin, i, j,
...
static void build_rr_xdir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
                           int nodes_per_chan, int nodes_direct, t_seg_details *seg_direct_x, 
                           int wire_to_ipin_switch, int cost_index_offset)
{
    ...
    for (itrack=0; itrack<nodes_direct; itrack++)
    {
        ...
        edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREX, itrack, nodes_direct, rr_node_indices);
        printf("in build_rr_xdir: inode=%d \n", inode);
			  if (i<nx)
			  ...
			  else
			  {
			      //nionio need to add for pad
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i+1, j, itrack, nodes_per_chan, io_rat, 
        ...
        //connects to my self
			  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREX, itrack+nodes_direct, nodes_direct, rr_node_indices);
        printf("in build_rr_xdir: inode=%d \n", inode);
			  if (i>0)
			      num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j, itrack, nodes_per_chan, nodes_direct, 
				                clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch);
			  else
			  {
			      //nionio needs to add for pad
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i, j, itrack, nodes_per_chan, io_rat, 
				                pads_to_direct, rr_node_indices, wire_to_ipin_switch);
			  }
        printf ("my num_edges=%d \n", num_edges);
			  alloc_and_load_edges_and_switches (inode, num_edges, edge_list_head);
...
static void build_rr_ydir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
                           int nodes_per_chan, int nodes_direct, t_seg_details *seg_direct_y, 
                           int wire_to_ipin_switch, int cost_index_offset)
{ 
    /* for right direction */
    int inode, itrack, seg_index, num_edges;
    ...
    for (itrack=0; itrack<nodes_direct; itrack++)
	  { 
			  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREY, itrack, nodes_direct, rr_node_indices);
        printf("in build_rr_ydir: inode=%d \n", inode);
			  if (j<ny)
			      num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j+1, itrack, nodes_per_chan, nodes_direct, 
				                clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch);
			  else
			  {
			      //nionio added
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i, j+1, itrack, nodes_per_chan, io_rat, 
				                pads_to_direct, rr_node_indices, wire_to_ipin_switch);
			  }
        printf ("my num_edges=%d \n", num_edges);
			  alloc_and_load_edges_and_switches (inode, num_edges, edge_list_head);
    	  ...
    	  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREY, itrack+nodes_direct, nodes_direct, rr_node_indices);
        printf("in build_rr_ydir: inode=%d \n", inode);
			  if (j>0)
			      num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j, itrack, nodes_per_chan, 
                        nodes_direct, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch);
			  else
			  {
			      //nionio added
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i, j, itrack, nodes_per_chan, io_rat, 
...
static int **alloc_and_load_clb_pin_to_direct (enum e_pin_type pin_type,
  ...
  // printf("pins_per_clb %d, nodes_direct %d\n", pins_per_clb, nodes_direct);
  tracks_connected_to_pin = (int**) alloc_matrix (0, pins_per_clb-1, 0, nodes_direct-1, sizeof(int));
  for (ipin=0; ipin<pins_per_clb; ipin++)
  {
    for (itrack=0; itrack<nodes_direct; itrack++)
      tracks_connected_to_pin[ipin][itrack]=0;
  }
  stepsize = nodes_direct/Fc;
  for (ipin=0;ipin<pins_per_clb;ipin++)
  {
    for (j=0;j<Fc;j++)
	{
       f_track = ipin + j * stepsize;
       itrack = (int) (f_track%nodes_direct);

  /* In case of weird roundoff effects */
       itrack = min(itrack, nodes_direct-1);
...
static int *alloc_and_load_pads_to_direct (int nodes_direct)
{
    ...
    for (ipad=0;ipad<io_rat;ipad++)
	    pads_to_direct[ipad]=ipad%nodes_direct;
