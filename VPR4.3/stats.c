...
static void load_channel_occupancies (int **chanx_occ, int **chany_occ, int **direx_occ, int **direy_occ); 
...
/* Determines how many tracks are used in each channel.                    */
 ...
 int i, j, max_occ, total_x, total_y, max_dir;
 float av_occ;
 ...
 int **direx_occ;   /* [1..nx-1][1..ny] */
 int **direy_occ;   /* [1..nx][1..ny-1] */


 chanx_occ = (int **) alloc_matrix (1, nx, 0, ny, sizeof(int));
 chany_occ = (int **) alloc_matrix (0, nx, 1, ny, sizeof(int));
 direx_occ = (int **) alloc_matrix (0, nx, 1, ny, sizeof(int));
 direy_occ = (int **) alloc_matrix (1, nx, 0, ny, sizeof(int));
 load_channel_occupancies (chanx_occ, chany_occ, direx_occ, direy_occ);
 ...
 for (j=0; j<=ny; j++) 
   {if(j>0)
     {for (i=1; i<nx; i++)
       max_dir = max (direx_occ[i][j], max_dir);
     
     printf("max direct %d\t %d\n", j, max_dir);
     }
   }
 ...
 for (i=0; i<=nx; i++) {
   if(i>0)
     {for (j=1; j<ny; j++)
       max_dir = max (direy_occ[i][j], max_dir);
...
/* Now go through each net and count the tracks and pins used everywhere */
 ...
 for (inet=0;inet<num_nets;inet++) {
    ...
    while (tptr != NULL) {
       ...
       else if (rr_type == DIREX) {
	 i = rr_node[inode].xlow;
	 j = rr_node[inode].ylow;
	 direx_occ[i][j]++;
       }
       
       else if (rr_type == DIREY) {
	 i = rr_node[inode].xlow;
	 j = rr_node[inode].ylow;
	 direy_occ[i][j]++;
       }
