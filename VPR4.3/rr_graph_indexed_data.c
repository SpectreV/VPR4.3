...
static void load_rr_indexed_data_base_costs(int nodes_per_chan,
		int nodes_direct, int ** rr_node_indices,
		enum e_base_cost_type base_cost_type, int wire_to_ipin_switch);

static float get_delay_normalization_fac(int nodes_per_chan, int nodes_direct,
		int io_rat, int ** rr_node_indices);

static float get_average_opin_delay(int **rr_node_indices, int nodes_per_chan,
...
void alloc_and_load_rr_indexed_data(t_segment_inf *segment_inf, int num_segment,
		int **rr_node_indices, int nodes_per_chan, int nodes_direct,
	...
	load_rr_indexed_data_T_values(CHANX_COST_INDEX_START, num_segment, CHANX,
			nodes_per_chan, nodes_direct, io_rat, rr_node_indices, segment_inf);
	...
	load_rr_indexed_data_T_values(CHANX_COST_INDEX_START + num_segment,
			num_segment, CHANY, nodes_per_chan, nodes_direct, io_rat,
			rr_node_indices, segment_inf);

	load_rr_indexed_data_base_costs(nodes_per_chan, nodes_direct,
			rr_node_indices, base_cost_type, wire_to_ipin_switch);
...
static void load_rr_indexed_data_base_costs(int nodes_per_chan,
	...
	if (base_cost_type == DEMAND_ONLY || base_cost_type == DELAY_NORMALIZED) {
		...
		rr_indexed_data[OPIN_COST_INDEX].base_cost = get_average_opin_delay(
				rr_node_indices, nodes_per_chan, nodes_direct, io_rat);
...
static float get_delay_normalization_fac(int nodes_per_chan, int nodes_direct,
	...
	// printf("the sum here %g\n", Tdel_sum);
	if (if_direct) {
		int i = nx + 1 / 2, j = ny + 1 / 2;
		while (clb[i][j].type == DSP) {
			i++;
			j++;
		}
		inode = get_rr_node_index(i, j, DIREX, 0,
				nodes_direct, nodes_direct, io_rat, rr_node_indices);
		cost_index = rr_node[inode].cost_index;
		frac_num_seg = clb_dist * rr_indexed_data[cost_index].inv_length;
		Tdel = frac_num_seg * rr_indexed_data[cost_index].T_linear
				+ frac_num_seg * frac_num_seg
						* rr_indexed_data[cost_index].T_quadratic;
		Tdel_sum += 4 * nodes_direct * Tdel / (float) clb_dist;
		return (Tdel_sum / (2. * nodes_per_chan + 4. * nodes_direct));
	} else
		return (Tdel_sum / (2.0 * nodes_per_chan));
}
...
static void load_rr_indexed_data_T_values(int index_start,
	...
	int itrack, iseg, inode, cost_index, iswitch, i;
	...
	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		inode = get_rr_node_index((nx + 1) / 2, (ny + 1) / 2, rr_type, itrack,
				nodes_per_chan, nodes_direct, io_rat, rr_node_indices);
		// printf("track %d inode %d cost_index %d\n",itrack, inode, cost_index);
		cost_index = rr_node[inode].cost_index;
	...
	if (if_direct) {
		for (i = 0; i < num_indices_to_load; i++) {
			if (segment_inf[i].type == DIRECT) {
				index = i;
				break;
			}
		}
		// printf("cost_start_index %d\n", index_start);
		//printf("direct index %d\n", index);
	}
	...
	for (cost_index = index_start;
		...
		if (num_nodes_of_index[cost_index] == 0) { /* Segments don't exist. */
			...
			//  printf("cost_index %d R %g C %g num %d\n", cost_index, R_total[cost_index], C_total[cost_index], num_nodes_of_index[cost_index]);
		...
		/*  printf("cost_index %d inv_length %g T_linear %g quadratic %g C_load %g\n", cost_index, rr_indexed_data[cost_index].inv_length, rr_indexed_data[cost_index].T_linear,
		 rr_indexed_data[cost_index].T_quadratic,  rr_indexed_data[cost_index].C_load);
		 */
	}

	if (if_direct) {
		dir = segment_inf[index].Rmetal;
		dic = segment_inf[index].Cmetal;
		rr_indexed_data[index + index_start].T_linear = 0.;
		rr_indexed_data[index + index_start].T_quadratic = 0.5 * dic * dir;
		rr_indexed_data[index + index_start].C_load = dic;
		/*printf("cost_index %d T_linear %g quadratic %g C_load %g\n", index+index_start, rr_indexed_data[index+index_start].T_linear,
		 rr_indexed_data[index+index_start].T_quadratic,  rr_indexed_data[index+index_start].C_load);
		 */
	}
