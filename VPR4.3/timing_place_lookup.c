...
static struct s_clb **clb_backup, ***stage_clb_backup;
...
static void assign_locations(enum e_block_types source_type, int source_x_loc,
		int source_y_loc, enum e_block_types sink_type, int sink_x_loc,
		int sink_y_loc);
...
static void generic_compute_matrix(float ***matrix_ptr,
		enum e_block_types source_type, enum e_block_types sink_type,
		int source_x, int source_y, int start_x, int end_x, int start_y,
		int end_y, struct s_router_opts router_opts,
		struct s_det_routing_arch det_routing_arch, t_segment_inf *segment_inf,
		t_timing_inf timing_inf);
...
static void assign_locations(enum e_block_types source_type, int source_x_loc,
	...
	if (source_type == CLB) {
		...
		if (is_folding) {
			stage_clb[0][source_x_loc][source_y_loc].u.block = SOURCE_BLOCK;
			stage_clb[0][source_x_loc][source_y_loc].occ += 1;
		}
	} else {
		...
		clb[source_x_loc][source_y_loc].occ += 1;
		if (is_folding) {
			isubblk = stage_clb[0][source_x_loc][source_y_loc].occ;
			stage_clb[0][source_x_loc][source_y_loc].u.io_blocks[isubblk] =
					SOURCE_BLOCK;
			stage_clb[0][source_x_loc][source_y_loc].occ += 1;
		}
	}
	if (sink_type == CLB) {
		...
		if (is_folding) {
			stage_clb[0][sink_x_loc][sink_y_loc].u.block = SINK_BLOCK;
			stage_clb[0][sink_x_loc][sink_y_loc].occ += 1;
		}
	} else {
		...
		clb[sink_x_loc][sink_y_loc].occ += 1;
		if (is_folding) {
			isubblk = stage_clb[0][sink_x_loc][sink_y_loc].occ;
			stage_clb[0][sink_x_loc][sink_y_loc].u.io_blocks[isubblk] =
					SINK_BLOCK;
			stage_clb[0][sink_x_loc][sink_y_loc].occ += 1;
		}
	}
...
static void compute_delta_clb_to_clb(struct s_router_opts router_opts,
	...
	//printf("start_x %d, start_y %d, end_x %d, end_y %d, source_x %d, source_y %d\n", start_x, start_y, end_x, end_y, source_x, source_y);
	...
	for (sink_x = start_x; sink_x <= end_x - 1; sink_x++) {
		for (sink_y = start_y; sink_y <= end_y - 1; sink_y++) {
			...
			// printf("source_x %d, source_y %d, sink_x %d, sink_y %d\n", source_x, source_y, sink_x, sink_y);
			...
			//printf("delta_x %d, delta_y %d, delay %g\n", delta_x, delta_y, delta_clb_to_clb[delta_x][delta_y]);
		}
	}
...
static void compute_delta_inpad_to_clb(struct s_router_opts router_opts,
	...
	generic_compute_matrix(&delta_inpad_to_clb, source_type, sink_type,
			source_x, source_y, start_x, end_x, start_y, end_y, router_opts,
...
static void compute_delta_clb_to_outpad(struct s_router_opts router_opts,
	...
	for (source_x = 1; source_x <= nx; source_x++) {
		for (source_y = 1; source_y <= ny; source_y++) {
			...
			delta_clb_to_outpad[delta_x][delta_y] = 0;
...
static void compute_delta_inpad_to_outpad(struct s_router_opts router_opts,
	...
	for (sink_y = 2; sink_y <= ny; sink_y++) {
		delta_y = abs(sink_y - source_y);
		delta_inpad_to_outpad[delta_x][delta_y] = assign_blocks_and_route_net(
				source_type, source_x, source_y, sink_type, sink_x, sink_y,
				router_opts, det_routing_arch, segment_inf, timing_inf);
...
void print_array(float** array, int x1, int x2, int y1, int y2) {
	...
	for (i = x1; i <= x2; i++) {
		for (j = y1; j <= y2; j++) {
			fprintf(lookup_dump, "%.2e\t", array[i][j]);
		}
		fprintf(lookup_dump, "\n");
	}
}
...
static void load_simplified_device(void) {
	...
	stage_clb_backup = stage_clb;

	stage_clb = (struct s_clb ***) alloc_matrix3(0, num_stage - 1, 0, nx + 1, 0,
			ny + 1, sizeof(struct s_clb));
	...
	for (i = 0; i < nx + 2; i++) {
		for (j = 0; j < ny + 2; j++) {
			if ((i == 0 && j == 0) || (i == 0 && j == ny + 1)
					|| (i == nx + 1 && j == 0) || (i == nx + 1 && j == ny + 1))
				clb[i][j].type = ILLEGAL;
