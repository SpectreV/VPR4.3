...
static void alloc_and_load_rr_graph (int **rr_node_indices, int *pin_to_opin,
       int ***clb_opin_to_tracks, struct s_ivec **tracks_to_clb_ipin, int** clb_pin_to_direct,
       int **pads_to_tracks, struct s_ivec *tracks_to_pads, int *pads_to_direct, int Fc_output,
       int Fc_input, int Fc_pad, int nodes_per_chan, int nodes_direct, enum e_route_type 
...
static void build_rr_clb (int **rr_node_indices, int Fc_output, int **clb_ipin_to_direct, int ***
         clb_opin_to_tracks, int nodes_per_chan, int nodes_direct, int i, int j,
         int delayless_switch, t_seg_details *seg_details_x, t_seg_details *seg_details_y);

static void build_rr_pads (int **rr_node_indices, int Fc_pad, int nodes_direct, int 
          **pads_to_tracks, int *pads_to_direct, int nodes_per_chan, int i, int j, int 
...
static void build_rr_xchan (int **rr_node_indices, enum e_route_type 
      ...
      tracks_to_pads, int i, int j, int nodes_per_chan, int nodes_direct, int io_rat,
...
static void build_rr_xdir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
      int nodes_per_chan, int nodes_direct, int io_rat, t_seg_details *seg_direct_x, int wire_to_ipin_switch, 
      int cost_index_offset);

static void build_rr_ydir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
      int nodes_per_chan, int nodes_direct, int io_rat, t_seg_details *seg_direct_y, int wire_to_ipin_switch, 
      int cost_index_offset);
 ...
 * for GLOBAL routing.                                                   */
 ...
 int **rr_node_indices;
 int Fc_output, Fc_input, Fc_pad, Fc_direct;
 int **pads_to_tracks;
 ...
 * anything on this side, the [ipin][iside][0] element is OPEN.        */
 ...
 if(if_direct)
   {seg_direct_x = alloc_and_load_direct_details (max(nodes_direct, io_rat), segment_inf,
						  det_routing_arch.num_segment);
   
   seg_direct_y = alloc_and_load_direct_details (max(nodes_direct, io_rat), segment_inf,
						 det_routing_arch.num_segment);
   }
#ifdef DEBUG
 ...
 * just rr_node_indices are allocated by this routine.                     */
 ...
 rr_node_indices = alloc_and_load_rr_node_indices (nodes_per_clb, 
         nodes_per_pad, nodes_per_chan, nodes_direct, io_rat, seg_details_x, seg_details_y);
 ...
 * track to connect to the same pin twice.                                   */
 ...
 pads_to_direct = alloc_and_load_pads_to_direct(stage_io_rat);  /* 2 direct links to pad */ 		  
 ...
 alloc_and_load_rr_graph (rr_node_indices, pin_to_opin, clb_opin_to_tracks,
       tracks_to_clb_ipin, clb_ipin_to_direct, pads_to_tracks, tracks_to_pads, pads_to_direct, Fc_output,
       Fc_input, Fc_pad, nodes_per_chan, nodes_direct, route_type, det_routing_arch, 
       seg_details_x, seg_details_y, seg_direct_x, seg_direct_y);
 ...
 alloc_and_load_rr_indexed_data (segment_inf, det_routing_arch.num_segment,
        rr_node_indices, nodes_per_chan, nodes_direct, det_routing_arch.wire_to_ipin_switch,
        base_cost_type);
...
static void alloc_and_load_rr_graph (int **rr_node_indices, int *pin_to_opin,
       int ***clb_opin_to_tracks, struct s_ivec **tracks_to_clb_ipin, int** clb_ipin_to_direct,
       int **pads_to_tracks, struct s_ivec *tracks_to_pads, int *pads_to_direct, int Fc_output,
       int Fc_input, int Fc_pad, int nodes_per_chan, int nodes_direct, enum e_route_type 
       route_type, struct s_det_routing_arch det_routing_arch, 
       t_seg_details *seg_details_x, t_seg_details *seg_details_y,
 ...
 * allocated as I fill in the graph.                                    */
 ...
 load_net_rr_terminals (rr_node_indices, nodes_per_chan, nodes_direct, io_rat);

 alloc_and_load_rr_clb_source (rr_node_indices, nodes_per_chan, nodes_direct, io_rat);
 ...
 for (i=0;i<=nx+1;i++) {
    for (j=0;j<=ny+1;j++) {
       ...
       if (clb[i][j].type == CLB||clb[i][j].type == MEM) {
         build_rr_clb (rr_node_indices, Fc_output, clb_ipin_to_direct, clb_opin_to_tracks, 
                 nodes_per_chan, nodes_direct, i, j, delayless_switch, seg_details_x, 
                 seg_details_y);
	       build_rr_xchan (rr_node_indices, route_type, tracks_to_clb_ipin, 
                         tracks_to_pads, i, j, nodes_per_chan, nodes_direct, io_rat, switch_block_type, 
		                     wire_to_ipin_switch, seg_details_x, seg_details_y, CHANX_COST_INDEX_START);
         build_rr_xdir (rr_node_indices, clb_ipin_to_direct, pads_to_direct, i, j, nodes_per_chan,
                        nodes_direct, io_rat, seg_direct_x, wire_to_ipin_switch, CHANX_COST_INDEX_START);
	       build_rr_ychan (rr_node_indices, route_type, tracks_to_clb_ipin,
                         tracks_to_pads, i, j, nodes_per_chan, nodes_direct, io_rat, switch_block_type, 
		                     wire_to_ipin_switch, seg_details_x, seg_details_y, CHANX_COST_INDEX_START + num_segment);
         build_rr_ydir (rr_node_indices, clb_ipin_to_direct, pads_to_direct, i, j, nodes_per_chan,
                        nodes_direct, io_rat, seg_direct_y, wire_to_ipin_switch, CHANX_COST_INDEX_START + num_segment);
     
       }

       else if (clb[i][j].type == IO) {
          build_rr_pads (rr_node_indices, Fc_pad, nodes_direct, pads_to_tracks,
                  pads_to_direct, nodes_per_chan, i, j, delayless_switch, seg_details_x, 
                  seg_details_y);
 ...
 * the rr index corresponding to the SOURCE (opin) or SINK (ipin) of pnum.  */
 ...
 for (inet=0;inet<num_nets;inet++) {
    ...
    for (ipin=0;ipin<net[inet].num_pins;ipin++) {
       ...
       inode = get_rr_node_index (i, j, rr_type, iclass, nodes_per_chan, nodes_direct, io_rat,  
                  rr_node_indices);
       //printf("inode is %d\n", inode);
...
static void build_rr_clb (int **rr_node_indices, int Fc_output,
			  ...
			  int nodes_per_chan, int nodes_direct, int i, int j, int 
       delayless_switch, t_seg_details *seg_details_x, t_seg_details 
       *seg_details_y) {
...
/* SOURCES and SINKS first.   */
 ...
 for (iclass=0;iclass<num_class;iclass++) {
    if (class_inf[iclass].type == DRIVER) {    /* SOURCE */
       //printf("source \n");
       inode = get_rr_node_index (i, j, SOURCE, iclass, nodes_per_chan, nodes_direct, io_rat, 
                 rr_node_indices);
       ...
       for (ipin=0;ipin<class_inf[iclass].num_pins;ipin++) {
              ...
              ((fo==4)&&(ipin%4==0)) )
	      {
            ...
            to_node = get_rr_node_index (i, j, OPIN, pin_num, nodes_per_chan, nodes_direct, io_rat, 
                  rr_node_indices);
       ...
       //printf("sink \n");
       inode = get_rr_node_index (i, j, SINK, iclass, nodes_per_chan, nodes_direct, io_rat, 
                 rr_node_indices);
...
/* Now do the pins.  */
 ...
 for (ipin=0;ipin<pins_per_clb;ipin++) {
    ...
    if (class_inf[iclass].type == DRIVER) {  /* OPIN */
       //printf("opin \n");
       inode = get_rr_node_index (i, j, OPIN, ipin, nodes_per_chan, nodes_direct, io_rat, 
                 rr_node_indices);
       
       edge_list_head = NULL;
	   //nionio: here handle clb opin to direct
       //printf("edges: \n");
       num_edges = get_clb_opin_connections (clb_ipin_to_direct, clb_opin_to_tracks, nodes_direct, ipin, i, j,
                   delayless_switch, Fc_output, seg_details_x, seg_details_y, &edge_list_head, 
       ...
       inode = get_rr_node_index (i, j, IPIN, ipin, nodes_per_chan, nodes_direct, io_rat, 
                 rr_node_indices);
        ...
        for (tmpPin=0;tmpPin<pins_per_clb;tmpPin++)
        {
            ...
            if (class_inf[tmpClass].type == DRIVER) numOpin++;
        }
	   ...
	   if (fi==4) numOpin=(ipin%4==0) ? 1+numOpin : numOpin;
	   if (fi==8) numOpin=(ipin%2!=1) ? 1+numOpin : numOpin;
	   if (fi==12) numOpin=(ipin%4!=1) ? 1+numOpin : numOpin;;
	   if (fi==16) numOpin++;
	   
       rr_node[inode].num_edges = numOpin;
       rr_node[inode].edges = (int *) my_chunk_malloc ((numOpin)*sizeof(int), 
          &rr_mem_chunk_list_head, &chunk_bytes_avail, &chunk_next_avail_mem);
       rr_node[inode].switches = (short *) my_chunk_malloc ((numOpin)*sizeof(short), 
          &rr_mem_chunk_list_head, &chunk_bytes_avail, &chunk_next_avail_mem);
       ...
       if( (fi==16) || 
	      ...
	      ((fi==4)&&(ipin%4==0)))
	   {
         to_node = get_rr_node_index (i, j, SINK, iclass, nodes_per_chan, nodes_direct, io_rat, 
                  rr_node_indices);
       ...
       //connects to OPIN
        ...
        {
            tmpClass = clb_pin_class[tmpPin];
            if (class_inf[tmpClass].type != DRIVER) continue;
			if ((fs==9)&&(tmpPin%4==ipin%4)) continue;
			if ((fs==6)&&(tmpPin%2==ipin%2)) continue;
			if ((fs==3)&&(tmpPin%4!=ipin%4)) continue;
            to_node = get_rr_node_index (i, j, OPIN, tmpPin, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
            rr_node[inode].edges[tmpCount] = to_node;
 ...
 * input or an output makes no difference.                                */
 ...
 for (ipad=0;ipad<io_rat;ipad++) {   /* For each pad at this (i,j) */
   ...
   /* Do SOURCE first.   */
    ...
    s_node = get_rr_node_index (i, j, SOURCE, ipad, nodes_per_chan, nodes_direct, io_rat, 
             rr_node_indices);
    //  printf("node is %d\n", s_node);   
    rr_node[s_node].num_edges = 1;
    p_node = get_rr_node_index (i, j, OPIN, ipad, nodes_per_chan, nodes_direct, io_rat,  
              rr_node_indices);
   ...
   /* Now do OPIN */
    ...
    num_edges = get_pad_opin_connections (pads_to_tracks, pads_to_direct, ipad, i, j, 
              Fc_pad, nodes_direct, delayless_switch, seg_details_x, seg_details_y, &edge_list_head, 
              nodes_per_chan, rr_node_indices);
   ...
   /* Now do SINK */
    ...
    s_node = get_rr_node_index (i, j, SINK, ipad, nodes_per_chan, nodes_direct, io_rat, 
             rr_node_indices);
    ...
    p_node = get_rr_node_index (i, j, IPIN, ipad, nodes_per_chan, nodes_direct, io_rat, 
             rr_node_indices);
...
/* First count number of edges and put the edges in a linked list. */
    ...
    if (j == 0) {                   /* Between bottom pads and clbs.  */
       num_edges = get_xtrack_to_clb_ipin_edges (istart, iend, j, itrack, TOP, 
                   &edge_list_head, tracks_to_clb_ipin, nodes_per_chan, nodes_direct, io_rat, 
                   rr_node_indices, seg_details_x, wire_to_ipin_switch);

       num_edges += get_xtrack_to_pad_edges (istart, iend, j, j, itrack, 
                    &edge_list_head, tracks_to_pads, nodes_per_chan, nodes_direct, io_rat,
                    rr_node_indices, seg_details_x, wire_to_ipin_switch);

       /* Only channels above exist. */

       num_edges += get_xtrack_to_ytracks (istart, iend, j, itrack, j+1,
                    &edge_list_head, nodes_per_chan, nodes_direct, io_rat, rr_node_indices, 
       ...
       num_edges = get_xtrack_to_clb_ipin_edges (istart, iend, j, itrack, 
                   BOTTOM, &edge_list_head, tracks_to_clb_ipin, nodes_per_chan, nodes_direct, io_rat,
                   rr_node_indices, seg_details_x, wire_to_ipin_switch);
       ...
       num_edges += get_xtrack_to_clb_ipin_edges (istart, iend, j, itrack, 
                    TOP, &edge_list_head, tracks_to_clb_ipin, nodes_per_chan, nodes_direct, io_rat, 
                    rr_node_indices, seg_details_x, wire_to_ipin_switch);
    ...
    inode = get_rr_node_index (i, j, CHANX, itrack, nodes_per_chan, nodes_direct, io_rat,  
             rr_node_indices);
...
/* First count number of edges and put the edges in a linked list. */
    ...
    if (i == 0) {                   /* Between leftmost pads and clbs.  */
       num_edges = get_ytrack_to_clb_ipin_edges (jstart, jend, i, itrack, 
                   RIGHT, &edge_list_head, tracks_to_clb_ipin, nodes_per_chan, nodes_direct, io_rat, 
       ...
       num_edges = get_ytrack_to_clb_ipin_edges (jstart, jend, i, itrack, 
                   LEFT, &edge_list_head, tracks_to_clb_ipin, nodes_per_chan, nodes_direct, io_rat, 
                   rr_node_indices, seg_details_y, wire_to_ipin_switch);
    ...
    inode = get_rr_node_index (i, j, CHANY, itrack, nodes_per_chan, nodes_direct, io_rat, 
             rr_node_indices);
...
static void build_rr_xdir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
                           int nodes_per_chan, int nodes_direct, int io_rat, t_seg_details *seg_direct_x, 
                           int wire_to_ipin_switch, int cost_index_offset)
{
  ...
  if ((i==0)||(i==nx))
  {
    ...
    for (itrack=0; itrack<max(nodes_direct, 2); itrack++)
    {
        ...
        edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREX, itrack, nodes_per_chan, nodes_direct, io_rat, rr_node_indices);
        //printf("in build_rr_xdir to right: inode=%d \n", inode);
			  
        if (i==0)
          num_edges = get_direct_to_clb_ipin_edges(&edge_list_head, i+1, j, itrack, nodes_per_chan, nodes_direct,
				            io_rat, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 1);
        else
			  {
			      //nionio need to add for pad
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i+1, j, itrack, nodes_per_chan, nodes_direct, io_rat, 
  ...
  else
  {
    for (itrack=0; itrack<nodes_direct; itrack++)
    {
        ...
        edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREX, itrack, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
			  num_edges = get_direct_to_clb_ipin_edges(&edge_list_head, i+1, j, itrack, nodes_per_chan, nodes_direct,
				            io_rat, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 0);
			  alloc_and_load_edges_and_switches (inode, num_edges, edge_list_head);
			  seg_index = seg_direct_x[itrack].index;
			  rr_node[inode].cost_index = cost_index_offset + seg_index;
			  rr_node[inode].occ = 0;
  ...
  { 
    ...
    for (itrack=0; itrack<max(nodes_direct, 2); itrack++)
    {
        ...
        /*inode = get_rr_node_index (i, j, DIREX, itrack+max(nodes_direct, io_rat), nodes_per_chan, nodes_direct, io_rat, 
                rr_node_indices);*/
			  inode = get_rr_node_index (i, j, DIREX, itrack+max(nodes_direct, 2), nodes_per_chan, nodes_direct, io_rat, 
                rr_node_indices);

        if (i==nx)
          num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j, itrack, nodes_per_chan, 
                      io_rat, nodes_direct, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 1);
        else
			  {
			      //nionio needs to add for pad
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i, j, itrack, nodes_per_chan, nodes_direct, io_rat, 
				                pads_to_direct, rr_node_indices, wire_to_ipin_switch);
			  }
  ...
  else
  {
    for (itrack=0; itrack<nodes_direct; itrack++)
    {
        //connects to my self
			  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREX, itrack+nodes_direct, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
			  num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j, itrack+nodes_direct, nodes_per_chan,
                    nodes_direct, io_rat, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 0);
			  alloc_and_load_edges_and_switches (inode, num_edges, edge_list_head);
			  rr_node[inode].cost_index = cost_index_offset + seg_index;
			  rr_node[inode].occ = 0;
			  rr_node[inode].capacity = 1;
...
static void build_rr_ydir (int **rr_node_indices, int **clb_ipin_to_direct, int *pads_to_direct, int i, int j,
                           int nodes_per_chan, int nodes_direct, int io_rat, t_seg_details *seg_direct_y, 
                           int wire_to_ipin_switch, int cost_index_offset)
{ 
    /* for right direction */
  ...
  if ((j==0)||(j==ny))
  {
    ...
    for (itrack=0; itrack<max(2,nodes_direct); itrack++)
	  { 
			  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREY, itrack, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
        //printf("in build_rr_ydir to up: inode=%d \n", inode);
			  if (j==0)
          num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j+1, itrack, nodes_per_chan, 
                   io_rat, nodes_direct, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 1);
        else
			  {
			      //nionio added
				    num_edges = get_direct_to_pad_edges (&edge_list_head, i, j+1, itrack, nodes_per_chan, nodes_direct, io_rat, 
				                pads_to_direct, rr_node_indices, wire_to_ipin_switch);
			  }
        //printf ("my num_edges=%d \n", num_edges);
			  alloc_and_load_edges_and_switches (inode, num_edges, edge_list_head);
  ...
  else
  {
    for (itrack=0; itrack<nodes_direct; itrack++)
	  { 
			  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREY, itrack, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
			  num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j+1, itrack+2*nodes_direct, nodes_per_chan,          
                    nodes_direct, io_rat, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 0);
    ...
    for (itrack=0; itrack<max(2,nodes_direct); itrack++)
	  {
        ...
        inode = get_rr_node_index (i, j, DIREY, itrack+max(nodes_direct,2), nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
  ...
  else
  {
    for (itrack=0; itrack<nodes_direct; itrack++)
	  {
    	  edge_list_head = NULL;
			  inode = get_rr_node_index (i, j, DIREY, itrack+nodes_direct, nodes_per_chan, nodes_direct, io_rat,  rr_node_indices);
        //printf("in build_rr_ydir to below: inode=%d \n", inode);
			  num_edges = get_direct_to_clb_ipin_edges (&edge_list_head, i, j, itrack+3*nodes_direct, nodes_per_chan, 
                        io_rat, nodes_direct, clb_ipin_to_direct, rr_node_indices, wire_to_ipin_switch, 0);
...
static int **alloc_and_load_clb_pin_to_direct (enum e_pin_type pin_type,
  ...
  tracks_connected_to_pin = (int**) alloc_matrix (0, pins_per_clb-1, 0, 4*nodes_direct-1, sizeof(int));
  for (ipin=0; ipin<pins_per_clb; ipin++)
  {
 ...
 * Set Fc and nodes_per_chan to 1 if you're doing a global routing graph.  */
 ...
 //nionio added
 if (nodes_per_chan==0) return NULL;
 ...
 * to make sure each track can connect to about the same number of ipins.   */
 ...
 for (i=0;i<num_phys_pins;i++) {
    ...
    //printf("pin:%d ,", ipin);
    if (nodes_per_chan%num_phys_pins !=0) num_conn=(nodes_per_chan/num_phys_pins)+1;
    else num_conn=(nodes_per_chan/num_phys_pins);
...
static int *alloc_and_load_pads_to_direct (int num_dir_to_pad)
{
    ...
    for (ipad=0;ipad<io_rat;ipad++)
	    pads_to_direct[ipad]=ipad%num_dir_to_pad;
