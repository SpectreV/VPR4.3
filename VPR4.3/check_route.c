		...
		if (is_folding) {
			if (current_stage != num_stage) {
				if ((inet < num_net_per_stage[current_stage - 1].begin)
						|| (inet >= num_net_per_stage[current_stage].begin))
					continue;
			} else if (inet < num_net_per_stage[current_stage - 1].begin)
				continue;
		}
			...
			if (rr_node[prev_node].type == SINK) {
				...
				if (!connects) {
					printf("Error in check_route while checking net %d.\n",
							inet);
					printf(
							"Non-adjacent segments in traceback from node %d [%d] to node %d [%d].\n",
							prev_node, rr_node[prev_node].type, inode,
							rr_node[inode].type);
					exit(1);
				}
...
static void check_sink(int inode, int inet, boolean *pin_done) {
	...
	if (!is_folding) {
		...
		if (stage_clb[current_stage - 1][i][j].type == CLB
				|| stage_clb[current_stage - 1][i][j].type == MEM) {
		...
		} else if (stage_clb[current_stage - 1][i][j].type == DSP) {
			bnum = stage_clb[current_stage - 1][i][j].u.block;
			for (ipin = 1; ipin < net[inet].num_pins; ipin++) {
				if (net[inet].blocks[ipin] == bnum) {
					blk_pin = net[inet].blk_pin[ipin];
			...
			bnum = stage_clb[current_stage - 1][i][j].u.io_blocks[ptc_num];
			for (ipin = 0; ipin < net[inet].num_pins; ipin++) {
				if (net[inet].blocks[ipin] == bnum) { /* Pad:  no pin class */
					ifound++;
					pin_done[ipin] = TRUE;
...
static void check_source(int inode, int inet) {
	...
	if (block[bnum].type == CLB || block[bnum].type == MEM) {
		...
		if (!is_folding) {
			if (clb[i][j].u.io_blocks[ptc_num] != bnum) {
				printf(
						"Error in check_source:  net SOURCE is at wrong pad (pad #%d)."
								"\n", ptc_num);
				exit(1);
			}
		} else {
			if (stage_clb[current_stage - 1][i][j].u.io_blocks[ptc_num]
					!= bnum) {
				printf(
						"Error in check_source:  net SOURCE is at wrong pad (pad #%d)."
								"\n", ptc_num);
...
static boolean check_adjacent(int from_node, int to_node) {
		...
		if (to_type == OPIN) {
			if (from_xlow == to_xlow && from_ylow == to_ylow) {
				if (clb[to_xlow][to_ylow].type == CLB
						|| clb[to_xlow][to_ylow].type == MEM) {
					iclass = clb_pin_class[to_ptc];
					if (iclass == from_ptc)
						num_adj++;
				} else if (clb[to_xlow][to_ylow].type == DSP) {
					iclass = dsp_pin_class[to_ptc];
					if (iclass == from_ptc)
						num_adj++;
				} else { /* IO block */
					if (to_ptc == from_ptc)
	...
	case OPIN:
		if (to_type == CHANX || to_type == CHANY || to_type == DIREX
				|| to_type == DIREY)
	...
	case IPIN:
		if (to_type == SINK && from_xlow == to_xlow && from_ylow == to_ylow) {
			if (clb[from_xlow][from_ylow].type == CLB
					|| clb[from_xlow][from_ylow].type == MEM) {
				iclass = clb_pin_class[from_ptc];
				if (iclass == to_ptc)
					num_adj++;
			} else if (clb[from_xlow][from_ylow].type == DSP) {
				iclass = dsp_pin_class[from_ptc];
				if (iclass == to_ptc)
					num_adj++;
			} else { /* OUTPAD */
				if (from_ptc == to_ptc)
		...
		//nionio added
		else if (to_type == OPIN && from_xlow == to_xlow
				&& from_ylow == to_ylow) {
			if (from_xlow == to_xlow && from_ylow == to_ylow
					&& (clb[to_xlow][to_ylow].type == CLB
							|| clb[to_xlow][to_ylow].type == MEM))
...
static int pin_and_chan_adjacent(int pin_node, int chan_node) {
	...
	if (clb[pin_x][pin_y].type == CLB || clb[pin_x][pin_y].type == MEM) {
		// printf("for clb\n");
		if (chan_type == CHANX) {
			//printf("Chanx \n");
		...
		} else if (chan_type == DIREX) {
			//printf("come here chanx, chan_xlow %d, chan_ylow %d, pin_x %d, pin_y %d\n", chan_xlow, chan_ylow, pin_x, pin_y);
			if (chan_ylow == pin_y)
				if (chan_xlow == pin_x || chan_xlow == pin_x - 1)
					num_adj++;
		} else if (chan_type == DIREY) {
			//printf("come here chany, chan_xlow %d, chan_ylow %d, pin_x %d, pin_y %d\n", chan_xlow, chan_ylow, pin_x, pin_y);
			if (chan_xlow == pin_x)
				if (chan_ylow == pin_y || chan_ylow == pin_y - 1)
					num_adj++;
		}
	}

	else if (clb[pin_x][pin_y].type == DSP) {
		if (chan_type == CHANX) {
			if (chan_ylow == pin_y + dsp_h - 1) { // CHANX above DSP
				for (i = 0; i < dsp_w; ++i) {
					if (dsp_pinloc[i][dsp_h - 1][TOP][pin_ptc] == 1) {
						num_adj++;
						break;
					}
				}
			} else if (chan_ylow == pin_y - 1) {
				for (i = 0; i < dsp_w; ++i) {
					if (dsp_pinloc[i][0][BOTTOM][pin_ptc] == 1) {
						num_adj++;
						break;
					}
				}
			...
			if (chan_xlow == pin_x + dsp_w - 1) {
				for (i = 0; i < dsp_h; ++i) {
					if (dsp_pinloc[dsp_w - 1][i][RIGHT][pin_ptc] == 1) {
						num_adj++;
						break;
					}
				}
			}
			if (chan_xlow == pin_x - 1) {
				for (i = 0; i < dsp_h; ++i) {
					if (dsp_pinloc[0][i][LEFT][pin_ptc] == 1) {
						num_adj++;
						break;
					}
				}
		...
		if (pin_y == 0) { /* Bottom row of pads. */
			if (chan_type == CHANX && chan_ylow == 0 && pin_x <= chan_xhigh
				...
				num_adj++;
			//added by nionio
			else if ((chan_type == DIREY) && (chan_xlow == pin_x)
					&& (chan_ylow == pin_y))
				num_adj++;
			...
			if (chan_type == CHANY && chan_xlow == 0 && pin_y <= chan_yhigh
				...
				num_adj++;
			//added by nionio
			else if ((chan_type == DIREX) && (chan_ylow == pin_y)
					&& (chan_xlow == pin_x))
				num_adj++;
...
static void recompute_occupancy_from_scratch(t_ivec **clb_opins_used_locally) {
	...
	for (iblk = 0; iblk < num_blocks; iblk++) {
		if (is_folding) {
			if (current_stage != num_stage) {
				if ((iblk < num_block_per_stage[current_stage - 1].begin)
						|| (iblk >= num_block_per_stage[current_stage].begin))
					continue;
			} else {
				if (iblk < num_block_per_stage[current_stage - 1].begin) {
					continue;
				...
				if (clb_pin_class[ipin] != iclass) {
					printf(
							"Error in check_locally_used_opins:  Block #%d (%s):\n"
									...
									"\trr_node #: %d.\n", iblk,
							block[iblk].name, iclass, clb_pin_class[ipin],
							inode);
					exit(1);
				}
			}
