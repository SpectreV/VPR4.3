 ...
 for (from_node=0;from_node<num_rr_nodes;from_node++) {
    ...
    case DIREX: case DIREY:
	  ...
	  case IPIN:
	     ...
	     if (from_rr_type == DIREX)
	       cblock_counted[rr_node[from_node].xlow] = FALSE;
	     else
	       cblock_counted[rr_node[from_node].ylow] = FALSE;
             break;
 ...
 * pull-up and pull-down paths.                                              */
 ...
 /* Target stage ratio = 4.  1 minimum width buffer, then nstage bigger *
  * ones.                                                                  */

    nstage = nint (log10 (R_minW_nmos / Rbuf) / log10 (4.));
    nstage = max (nstage, 1);
    stage_ratio = pow (R_minW_nmos / Rbuf, 1. / (float) nstage);
