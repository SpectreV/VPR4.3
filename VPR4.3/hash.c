...
static int hash_value_new (char* name, int stage);
 ...
 * created.                                                                 */
 ...
 int i;  
 struct s_hash *h_ptr, *prev_ptr, *tmp_ptr; 
...
/* Name string wasn't in the hash table.  Add it. */ 
 ...
 h_ptr->name = (char *) my_malloc (256 * sizeof(char));
 if (prev_ptr == NULL) {
    hash_table[i] = h_ptr;
 }
 else {
 ...
 * created.                                                                 */
 ...
 i = hash_value_new(name, stage); 
 //printf("hash value is %d\n", i);
 prev_ptr = NULL;
 h_ptr = hash_table[i];
 while (h_ptr != NULL) { 
    if ((strcmp (h_ptr->name, name) == 0)&&(h_ptr->stage == stage)) {
       h_ptr->count++;
       return (h_ptr);
    }
    //printf("come here\n");
