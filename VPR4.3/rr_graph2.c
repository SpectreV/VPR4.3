...
/* added by Wei */
static int ***direx_rr_indices; /* [1..nx-1][1..ny-1][0..2*nodes_direct-1] */
static int ***direy_rr_indices; /* [1..nx-1][1..ny-1][0..2*nodes_direct-1] */

/************************** Subroutines local to this module ****************/

static int load_chanx_rr_indices(t_seg_details *seg_details_x,
		int nodes_per_chan, int start_index, int i, int j);

static int load_chany_rr_indices(t_seg_details *seg_details_y,
		int nodes_per_chan, int start_index, int i, int j);

static int load_direx_rr_indices(int nodes_per_chan, int start_index, int i,
		int j);

static int load_direy_rr_indices(int nodes_per_chan, int start_index, int i,
		int j);
...
t_seg_details *alloc_and_load_seg_details(int nodes_per_chan,
		t_segment_inf *segment_inf, int num_seg_types, int max_len) {
	...
	for (i = 0; i < num_seg_types; i++) {
		...
		for (itrack = 0; itrack < ntracks; itrack++) {
			...
			for (j = 0; j < num_sb; j++) {
				index = nint (j * sb_step + itrack * sb_off_step)
						% (length + 1);
...
int get_closest_seg_start(t_seg_details *seg_details, int itrack, int seg_num,
	...
	if (seg_details[itrack].longline) {
		...
		closest_start = seg_num
				- (seg_num + chan_num - start + length) % length;
...
int get_dsp_opin_connections(int *****dsp_opin_to_tracks, int nodes_direct,
		int ipin, int i, int j, int delayless_switch, int Fc_dsp,
	...
	int k, pin_num, map_pin;
	int edge_pin, not_edge_pin;
	int x_off = clb[i][j].x_off;
	int y_off = clb[i][j].y_off;
	...
	if (nodes_per_chan != 0) {
		for (iside = 0; iside <= 3; iside++) {
			if (dsp_opin_to_tracks[ipin][x_off][y_off][iside][0] != OPEN) {
				...
				if (tr_j < 0 || tr_j > ny)
					continue;
				if (CHANX == to_rr_type && tr_i < 1)
					continue;
				if (CHANY == to_rr_type && tr_j < 1)
					continue;

				for (iconn = 0; iconn < Fc_dsp; iconn++) {
					itrack =
							dsp_opin_to_tracks[ipin][x_off][y_off][iside][iconn];
					...
					if (cbox_exists) {
						to_node = get_rr_node_index(tr_i, tr_j, to_rr_type,
								itrack, nodes_per_chan, nodes_direct, io_rat,
								rr_node_indices);
						// printf("segment connect %d\n", to_node);
						edge_list_head = insert_in_edge_list(edge_list_head,
								to_node, seg_details[itrack].opin_switch,
								&free_edge_list_head);
						num_conn++;
					}
				}
...
int get_clb_opin_connections(int **clb_ipin_to_direct,
		int ***clb_opin_to_tracks, int nodes_direct, int ipin, int i, int j,
		int delayless_switch, int Fc_output, t_seg_details *seg_details_x,
		t_seg_details *seg_details_y, t_linked_edge **edge_list_ptr,
	...
	// printf("come to direct links \n");
	{
		for (itrack = 0; itrack < 4 * nodes_direct; itrack++) {
			if (clb_ipin_to_direct[ipin][itrack] == 1) {
				//nionio modified
				map_pin = itrack;
				edge_pin = not_edge_pin = map_pin;
				k = 0;
				while (map_pin >= nodes_direct) {
					k++;
					map_pin -= nodes_direct;
				}

				while (not_edge_pin >= nodes_direct)
					not_edge_pin -= nodes_direct;
				//nionio 7/11
				//while (edge_pin >= io_rat)
				//    edge_pin-=io_rat;
				//nionio 8/14
				/*while (edge_pin >= stage_io_rat)
				 edge_pin-=stage_io_rat;*/
				while (edge_pin >= nodes_direct)
					edge_pin -= nodes_direct;
				...
				if (k == 0) {
					...
					/*if (i==1)
					 //nionio 7/11
					 //pin_num = max(stage_io_rat, nodes_direct) + edge_pin;
					 //pin_num = max(3, nodes_direct) + edge_pin;
					 else */
					pin_num = nodes_direct + not_edge_pin;
				} else if (k == 1) {
					tr_i = i;
					tr_j = j;
					to_rr_type = DIREX;
					...
					/*if (j==1)
					 //nionio 7/11
					 pin_num = max(stage_io_rat, nodes_direct) + edge_pin;
					 //pin_num = max(2, nodes_direct) + edge_pin;
					 else*/
					pin_num = nodes_direct + not_edge_pin;
				} else if (k == 3) {
					tr_i = i;
				...
				if ((i == 1) && (k != 0)) {
					...
					to_rr_type = DIREX;
					//nionio 7/11
					//pin_num = max(io_rat, nodes_direct)+edge_pin;
					//pin_num = max(stage_io_rat, nodes_direct)+edge_pin;
					//nionio 8/14
					pin_num = nodes_direct + edge_pin;
					to_node = get_rr_node_index(tr_i, tr_j, to_rr_type, pin_num,
							nodes_per_chan, nodes_direct, io_rat,
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
...
//nionio modified
int get_pad_opin_connections(int **pads_to_tracks, int *pads_to_direct,
		int ipad, int i, int j, int Fc_pad, int nodes_direct,
	...
	//nionio 7/11
	//int dir_num=max(io_rat, nodes_direct);
...
boolean is_cbox(int seg_num, int chan_num, int itrack,
		t_seg_details *seg_details) {
...
int **alloc_and_load_rr_node_indices(int nodes_per_clb, int nodes_per_pad,
	...
	//nionio modified
	/*direx_rr_indices = (int ***) alloc_matrix3 (1, nx-1, 1, ny, 0, 2*nodes_direct-1, sizeof (int));
	 direy_rr_indices = (int ***) alloc_matrix3 (1, nx, 1, ny-1, 0, 2*nodes_direct-1, sizeof (int));*/
	/*if (nodes_direct > io_rat)
	 {
	 direx_rr_indices = (int ***) alloc_matrix3 (0, nx, 1, ny, 0, 2*nodes_direct-1, sizeof (int));
	 direy_rr_indices = (int ***) alloc_matrix3 (1, nx, 0, ny, 0, 2*nodes_direct-1, sizeof (int));
	 }
	 else
	 {
	 direx_rr_indices = (int ***) alloc_matrix3 (0, nx, 1, ny, 0, 2*io_rat-1, sizeof (int));
	 direy_rr_indices = (int ***) alloc_matrix3 (1, nx, 0, ny, 0, 2*io_rat-1, sizeof (int));
	 }*/
	...
	for (i = 0; i <= nx + 1; i++) {
		for (j = 0; j <= ny + 1; j++) {
			...
			if (clb[i][j].type == CLB || clb[i][j].type == MEM) {
				...
				if (if_direct) {
					//nionio modified
					/*if ((i==nx)&&(io_rat>nodes_direct))
					 index = load_direx_rr_indices (io_rat, index, i, j);
					 else
					 index = load_direx_rr_indices (nodes_direct, index, i, j);
					 if ((j==ny)&&(io_rat>nodes_direct))
					 index = load_direy_rr_indices (io_rat, index, i, j);
					 else
				...
				printf(
						"Error in alloc_and_load_rr_node_indices.  Unexpected clb"
								" type.\n");
				exit(1);
			}
		}
...
void free_rr_node_indices(int **rr_node_indices) {
	...
	free_matrix3(direx_rr_indices, 0, nx, 1, ny, 0, sizeof(int));
	free_matrix3(direy_rr_indices, 1, nx, 0, ny, 0, sizeof(int));
}
...
static int load_chanx_rr_indices(t_seg_details *seg_details_x,
	...
	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		...
		//printf("itrack %d, istart %d, iend %d, rr_index %d\n", itrack, istart, iend, rr_index);
		rr_index++;
	}
...
static int load_chany_rr_indices(t_seg_details *seg_details_y,
	...
	for (itrack = 0; itrack < nodes_per_chan; itrack++) {
		...
		//printf("itrack %d, jstart %d, jend %d, rr_index %d\n", itrack, jstart, jend, rr_index);
		rr_index++;
	}
...
static int load_direx_rr_indices(int nodes_direct, int start_index, int i,
		int j) {

	int rr_index, itrack;
	rr_index = start_index;

	for (itrack = 0; itrack < 2 * nodes_direct; itrack++) {
		direx_rr_indices[i][j][itrack] = rr_index;
		rr_index++;
	}
...
static int load_direy_rr_indices(int nodes_direct, int start_index, int i,
		int j) {
	...
	for (itrack = 0; itrack < 2 * nodes_direct; itrack++) {
		direy_rr_indices[i][j][itrack] = rr_index;
		rr_index++;
	}
...
int get_rr_node_index(int i, int j, t_rr_type rr_type, int ioff,
		int nodes_per_chan, int nodes_direct, int io_rat, int **rr_node_indices) {
	...
	case DSP:
		...
		case SOURCE:
			assert(ioff < num_class);
			assert(dsp_class_inf[ioff].type == DRIVER);
		...
		case SINK:
			assert(ioff < num_class);
			assert(dsp_class_inf[ioff].type == RECEIVER);

			index += ioff;
			return (index);

		case OPIN:
			assert(ioff < pins_per_dsp);
			iclass = dsp_pin_class[ioff];
			assert(dsp_class_inf[iclass].type == DRIVER);

			index += dsp_num_class + ioff;
			return (index);

		case IPIN:
			assert(ioff < pins_per_dsp);
			iclass = dsp_pin_class[ioff];
			assert(dsp_class_inf[iclass].type == RECEIVER);
...
int get_seg_end(t_seg_details *seg_details, int itrack, int seg_start,
		int chan_num, int max_end) {
...
int get_direct_to_clb_ipin_edges(t_linked_edge **edge_list_ptr, int i, int j,
		int itrack, int nodes_per_chan, int nodes_direct, int io_rat,
		int **clb_ipin_to_direct, int **rr_node_indices,
		int wire_to_ipin_switch, boolean connect_all) {
	int ipin, iclass, to_node, numt;
	t_linked_edge *edge_list_head;
	edge_list_head = *edge_list_ptr;
			...
			for (ipin = 0; ipin < pins_per_clb; ipin++) {
				...
				if (class_inf[iclass].type == RECEIVER) {
					to_node = get_rr_node_index(i, j, IPIN, ipin,
							...
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, wire_to_ipin_switch, &free_edge_list_head);
					num_conn++;
				}
			}
		}
	...
	*edge_list_ptr = edge_list_head;
	//printf("num of conn %d", num_conn);
	return (num_conn);
}

//nionio added
int get_direct_to_pad_edges(t_linked_edge **edge_list_ptr, int i, int j,
		int itrack, int nodes_per_chan, int nodes_direct, int io_rat,
		int *pads_to_direct, int **rr_node_indices, int wire_to_ipin_switch) {
	/* This function counts the number of pads the direct link connects to *
	 and adds the connections into the linked list                       */
	int ipad, num_conn, to_node;
	...
	for (ipad = 0; ipad < io_rat; ipad++) {
		if (pads_to_direct[ipad] == itrack) {
			to_node = get_rr_node_index(i, j, IPIN, ipad, nodes_per_chan,
...
int get_xtrack_to_clb_ipin_edges(int tr_istart, int tr_iend, int tr_j,
		int itrack, int iside, t_linked_edge **edge_list_ptr,
		struct s_ivec **tracks_to_clb_ipin,
		struct s_ivec ****tracks_to_dsp_ipin, int nodes_per_chan,
		int nodes_direct, int io_rat, int **rr_node_indices,
	...
	if (iside == BOTTOM) {
		...
		clb_j = tr_j + 1;
		iside = BOTTOM; // change to clb's perspective
	} else {
	...
	for (i = tr_istart; i <= tr_iend; i++) {
		if (is_cbox(i, tr_j, itrack, seg_details_x)) {
			if (clb[i][clb_j].type == CLB || clb[i][clb_j].type == MEM) {
				...
				for (iconn = 0; iconn < max_conn; iconn++) {
					...
					to_node = get_rr_node_index(i, clb_j, IPIN, ipin,
							nodes_per_chan, nodes_direct, io_rat,
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, wire_to_ipin_switch, &free_edge_list_head);
				...
				num_conn += max_conn;
			} else if (clb[i][clb_j].type == DSP) {
				x_off = clb[i][clb_j].x_off;
				y_off = clb[i][clb_j].y_off;
				max_conn =
						tracks_to_dsp_ipin[itrack][x_off][y_off][iside].nelem;
				for (iconn = 0; iconn < max_conn; iconn++) {
					ipin =
							tracks_to_dsp_ipin[itrack][x_off][y_off][iside].list[iconn];
					to_node = get_rr_node_index(i - x_off, clb_j - y_off, IPIN,
							ipin, nodes_per_chan, nodes_direct, io_rat,
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, wire_to_ipin_switch, &free_edge_list_head);
				}
...
int get_ytrack_to_clb_ipin_edges(int tr_jstart, int tr_jend, int tr_i,
	...
	if (iside == LEFT) {
		clb_i = tr_i;
		iside = RIGHT; // change to clb's perspective
	} else if (iside == RIGHT) {
		clb_i = tr_i + 1;
		iside = LEFT;
	} else {
	...
	for (j = tr_jstart; j <= tr_jend; j++) {
		if (is_cbox(j, tr_i, itrack, seg_details_y)) {
			if (clb[clb_i][j].type == CLB || clb[clb_i][j].type == MEM) {
				max_conn = tracks_to_clb_ipin[itrack][iside].nelem;
				for (iconn = 0; iconn < max_conn; iconn++) {
					ipin = tracks_to_clb_ipin[itrack][iside].list[iconn];
					to_node = get_rr_node_index(clb_i, j, IPIN, ipin,
							nodes_per_chan, nodes_direct, io_rat,
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, wire_to_ipin_switch, &free_edge_list_head);
				...
				num_conn += max_conn;
			} else if (clb[clb_i][j].type == DSP) {
				x_off = clb[clb_i][j].x_off;
				y_off = clb[clb_i][j].y_off;
				max_conn =
						tracks_to_dsp_ipin[itrack][x_off][y_off][iside].nelem;
				for (iconn = 0; iconn < max_conn; iconn++) {
					ipin =
							tracks_to_dsp_ipin[itrack][x_off][y_off][iside].list[iconn];
					to_node = get_rr_node_index(clb_i - x_off, j - y_off, IPIN,
							ipin, nodes_per_chan, nodes_direct, io_rat,
							rr_node_indices);
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, wire_to_ipin_switch, &free_edge_list_head);
...
int get_xtrack_to_pad_edges(int tr_istart, int tr_iend, int tr_j, int pad_j,
		int itrack, t_linked_edge **edge_list_ptr,
		struct s_ivec *tracks_to_pads, int nodes_per_chan, int nodes_direct,
		int io_rat, int **rr_node_indices, t_seg_details *seg_details_x,
	...
	for (i = tr_istart; i <= tr_iend; i++) {
		if (is_cbox(i, tr_j, itrack, seg_details_x)) {
			for (iconn = 0; iconn < max_conn; iconn++) {
				...
				to_node = get_rr_node_index(i, pad_j, IPIN, ipad,
						nodes_per_chan, nodes_direct, io_rat, rr_node_indices);
				edge_list_head = insert_in_edge_list(edge_list_head, to_node,
						wire_to_ipin_switch, &free_edge_list_head);
			}
...
int get_ytrack_to_pad_edges(int tr_jstart, int tr_jend, int tr_i, int pad_i,
		int itrack, t_linked_edge **edge_list_ptr,
		struct s_ivec *tracks_to_pads, int nodes_per_chan, int nodes_direct,
		int io_rat, int **rr_node_indices, t_seg_details *seg_details_y,
		int wire_to_ipin_switch) {
	...
	for (j = tr_jstart; j <= tr_jend; j++) {
		if (is_cbox(j, tr_i, itrack, seg_details_y)) {
			for (iconn = 0; iconn < tracks_to_pads[itrack].nelem; iconn++) {
				ipad = tracks_to_pads[itrack].list[iconn];
				to_node = get_rr_node_index(pad_i, j, IPIN, ipad,
						nodes_per_chan, nodes_direct, io_rat, rr_node_indices);
				edge_list_head = insert_in_edge_list(edge_list_head, to_node,
						wire_to_ipin_switch, &free_edge_list_head);
			}
...
int get_xtrack_to_ytracks(int from_istart, int from_iend, int from_j,
		int from_track, int to_j, t_linked_edge **edge_list_ptr,
		int nodes_per_chan, int nodes_direct, int io_rat, int **rr_node_indices,
		t_seg_details *seg_details_x, t_seg_details *seg_details_y,
	...
	for (i = from_istart; i <= from_iend; i++) {
		...
		for (iconn = 0; iconn < conn_tracks.nelem; iconn++) {
			...
			is_y_sbox = is_sbox(to_j, i - 1, to_track, seg_details_y,
					yconn_to_above);
			to_node_switch = seg_details_y[to_track].wire_switch;

			get_switch_type(is_x_sbox, is_y_sbox, from_node_switch,
			...
			if (switch_types[0] != OPEN) {
				...
				if (!rr_edge_done[to_node]) { /* Not a repeat edge. */
					...
					edge_list_head = insert_in_edge_list(edge_list_head,
							to_node, switch_types[0], &free_edge_list_head);

					if (switch_types[1] != OPEN) { /* A second edge. */
						edge_list_head = insert_in_edge_list(edge_list_head,
								to_node, switch_types[1], &free_edge_list_head);
						num_conn++;
					}
				}
			}
		...
		for (iconn = 0; iconn < conn_tracks.nelem; iconn++) {
			to_track = conn_tracks.list[iconn];
			is_y_sbox = is_sbox(to_j, i, to_track, seg_details_y,
					yconn_to_above);
			to_node_switch = seg_details_y[to_track].wire_switch;
...
int get_ytrack_to_xtracks(int from_jstart, int from_jend, int from_i,
		int from_track, int to_i, t_linked_edge **edge_list_ptr,
		int nodes_per_chan, int nodes_direct, int io_rat, int **rr_node_indices,
		t_seg_details *seg_details_x, t_seg_details *seg_details_y,
		enum e_switch_block_type switch_block_type) {
	...
	for (j = from_jstart; j <= from_jend; j++) {
		...
		for (iconn = 0; iconn < conn_tracks.nelem; iconn++) {
			to_track = conn_tracks.list[iconn];
			is_x_sbox = is_sbox(to_i, j - 1, to_track, seg_details_x,
					xconn_to_right);
			to_node_switch = seg_details_x[to_track].wire_switch;

			get_switch_type(is_y_sbox, is_x_sbox, from_node_switch,
					to_node_switch, switch_types);
		...
		for (iconn = 0; iconn < conn_tracks.nelem; iconn++) {
			to_track = conn_tracks.list[iconn];
			is_x_sbox = is_sbox(to_i, j, to_track, seg_details_x,
					xconn_to_right);
			to_node_switch = seg_details_x[to_track].wire_switch;
...
int get_xtrack_to_xtrack(int from_i, int j, int from_track, int to_i,
		t_linked_edge **edge_list_ptr, int nodes_per_chan, int nodes_direct,
		int io_rat, int **rr_node_indices, t_seg_details *seg_details_x,
		enum e_switch_block_type switch_block_type) {
	...
	is_from_sbox = is_sbox(from_i, j, from_track, seg_details_x,
			from_goes_right);
	conn_tracks = get_switch_box_tracks(from_i, j, from_track, CHANX, to_i, j,
			CHANX, switch_block_type, nodes_per_chan);
...
boolean is_sbox(int seg_num, int chan_num, int itrack,
		t_seg_details *seg_details, boolean above_or_right) {
	...
	if (!longline) {
		...
		seg_offset = (seg_num + chan_num - start + above_or_right)
				% (length + 1);
	}
