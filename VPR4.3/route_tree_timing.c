 ...
 * a pointer to the rt_node of the SINK that it adds to the routing.        */
 ...
 if (subtree_parent_rt_node != NULL) {  /* Parent exists. */
    ...
    //printf("inode %d, Tdel_start %g\n", subtree_parent_rt_node->inode, Tdel_start);
    iswitch = unbuffered_subtree_rt_root->parent_switch;
    //printf("parent switch %d\n", iswitch);
    Tdel_start += switch_inf[iswitch].R * 
                   unbuffered_subtree_rt_root->C_downstream;
    //printf("the delay here %g\n", Tdel_start);
    Tdel_start += switch_inf[iswitch].Tdel;
    //printf("the delay here %g, with switch delay %g\n", Tdel_start, switch_inf[iswitch].Tdel);
 } 
...
/* For all "new" nodes in the path */
 ...
 while (rr_node_route_inf[inode].prev_node != NO_PREVIOUS) {
    ...
    if (switch_inf[iswitch].buffered == FALSE)
       {C_downstream += rr_node[inode].C;
       //printf("unbuffered\n");
       //printf("node %d, C %g, total_C %g\n", inode, rr_node[inode].C, C_downstream);
       }
    else
       {C_downstream = rr_node[inode].C;
       //printf("buffered\n");
 ...
 * correct value.                                                           */
 ...
 //printf("switch type %d R %g node_R %g\n", iswitch, switch_inf[iswitch].R, rr_node[inode].R);

 if (switch_inf[iswitch].buffered == FALSE)
   {//printf("not buffered\n");
   R_upstream += parent_rt_node->R_upstream;
   //printf("parent upstream R %g\n", parent_rt_node->R_upstream);
   }

 //printf("total upstream R %g\n", R_upstream);
 ...
 * recursion, etc.                                                          */
 ...
 while (linked_rt_edge != NULL) {   /* While SINK not reached. */
    ...
    if (switch_inf[iswitch].buffered) 
      {//printf("buffered \n");
      R_upstream = switch_inf[iswitch].R + rr_node[inode].R;
      //printf("switch R %g, inode R %g\n", switch_inf[iswitch].R, rr_node[inode].R);
      }
    else
      {R_upstream += switch_inf[iswitch].R + rr_node[inode].R;
      //printf("unbuffered \n");
 ...
 * values are affected by the new path's addition.                          */
 ...
 // printf("node is %d with C %g\n", rt_node->inode, C_downstream_addition);
 parent_rt_node = rt_node->parent_node;
 iswitch = rt_node->parent_switch;
 ...
 while (parent_rt_node != NULL && switch_inf[iswitch].buffered == FALSE) {
   //printf("No buffer \n");
    ...
    //printf("parent node %d, C %g\n", rt_node->inode, rt_node->C_downstream);
 ...
 * first pre-order traversal).                                              */
 ...
 while (linked_rt_edge != NULL) {
    ...
    //printf("the child node %d\n", child_node->inode);
    Tchild = Tdel + switch_inf[iswitch].R * child_node->C_downstream;
    //printf("The R %g, C %g, Tdel %g\n", switch_inf[iswitch].R, child_node->C_downstream, Tchild);
    Tchild += switch_inf[iswitch].Tdel;     /* Intrinsic switch delay. */
    //printf("Tdel with switch Tdel: %g\n", switch_inf[iswitch].Tdel);
